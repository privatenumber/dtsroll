...
const createExternalizePlugin = (configuredExternals) => {
  ...
  const externalizePlugin = {
    ...
    async resolveId(id, importer, options) {
      console.log("[dtsroll] 0 externalize resolveId", { id, importer, options });
      const packageName = !isPath(id) && getPackageName(id);
      if (packageName) {
        console.log("[dtsroll]", 1);
      ...
      if (resolved) {
        console.log("[dtsroll]", 2);
        if (packageName) {
          console.log("[dtsroll]", 3);
        ...
        if (
          ...
          console.log("[dtsroll]", 4);
          importPath.set(resolved.id, importer);
        }
        return resolved;
      }
      if (packageName) {
        console.log("[dtsroll]", 5);
...
const dtsroll = async ({
  ...
  console.dir(built.output, { depth: 3, maxArrayLength: null });
  let outputSize = 0;
  const outputEntries = [];
  const outputChunks = [];
