...
describe('errors', ({ test }) => {
	...
	test('Outside of cwd', async () => {
		await using fixture = await createFixture({});

		const spawned = await dtsroll(fixture.path, ['../non-existent-file.d.ts']);
		expect('exitCode' in spawned && spawned.exitCode === 1).toBe(true);
		expect(spawned.stdout).toContain('Ignoring file outside of cwd');
	});
	...
	test('Invalid input shouldnt fallback to package.json', async () => {
		...
		expect('exitCode' in spawned && spawned.exitCode === 1).toBe(true);
		expect(spawned.stdout).toContain('Ignoring non-d.ts input');
		expect(spawned.stderr).toContain('Failed to build: No input files');
...
describe('removeBundled', ({ test }) => {
	test('Removes bundled', async () => {
		await using fixture = await createFixture({
			...fixtures.multipleEntryPoints,
			'package.json': JSON.stringify({
				types: 'dist/index.d.ts',
		...
		const spawned = await dtsroll(fixture.path, ['--removeBundled']);
		expect('exitCode' in spawned).toBe(false);

		const bundledModuleExists = await fixture.exists('dist/dir/common.d.ts');
		expect(bundledModuleExists).toBe(false);
	});

	test('Doesnt remove bundled in dry mode', async () => {
		await using fixture = await createFixture({
			...fixtures.multipleEntryPoints,
			'package.json': JSON.stringify({
				types: 'dist/index.d.ts',
		...
		const spawned = await dtsroll(fixture.path, ['--removeBundled', '--dry']);
		expect('exitCode' in spawned).toBe(false);

		const bundledModuleExists = await fixture.exists('dist/dir/common.d.ts');
		expect(bundledModuleExists).toBe(true);
	});
});
